...
protected:
	...
	bool SetMotion(int func) {
		...
		SendBuf[0] = func;
		SendBuf[11] = 2.56;
		if (this->SendMsgRight((char*)SendBuf, this->SendLen))return true;
		else return false;
	}
	//运动指令发送
	...
	bool SetMotion(int func, float* buf, int velA = 90, int vel = 500, bool FlagRad = true) {
		if ((!this->m_isSocketFlag_right) || (!this->enable) || (!this->script)) return false;
		float SendBuf[12] = { 0 };
		SendBuf[0] = func;
		SendBuf[11] = 2.56;
		for (int i = 0; i < 6; i++) {
	...
	bool KebaAnalyse() {
		...
		float recvbuf[16] = { 0 };
		if (!this->RecvMsgRight((char*)recvbuf, this->RecvLen)) {
			return false;
		}
